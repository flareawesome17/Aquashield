...
export default function EmployeeTable() {
  ...
  const handleSave = async (updatedEmployee: Employee) => {
    try {
      await updateDoc(doc(db, "employees", updatedEmployee.id), {
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {filteredEmployees.length > 0 ? (
              filteredEmployees.map((emp) => (
                <TableRow key={emp.id}>
                  <TableCell className="px-5 py-3 text-start font-medium text-gray-700 text-theme-xs dark:text-gray-400">{emp.fname} {emp.lname}</TableCell>
